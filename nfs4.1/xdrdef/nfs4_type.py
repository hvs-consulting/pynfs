...
class netloc4:
    ...
    #         netaddr4 nl_addr;
    # };
    def __init__(self, nl_type=None, nl_name=None, nl_url=None, nl_addr=None):
        self.nl_type = nl_type
        self.nl_name = nl_name
        self.nl_url = nl_url
        self.nl_addr = nl_addr

    switch = property(lambda s: {const.NL4_NAME:s.nl_name,const.NL4_URL:s.nl_url,const.NL4_NETADDR:s.nl_addr,}[s.nl_type])
    ...
    def __repr__(self):
        out = []
        if self.nl_type is not None:
            out += ['nl_type=%s' % const.netloc_type4.get(self.nl_type, self.nl_type)]
        if self.nl_name is not None:
            out += ['nl_name=%s' % repr(self.nl_name)]
        if self.nl_url is not None:
            out += ['nl_url=%s' % repr(self.nl_url)]
        if self.nl_addr is not None:
            out += ['nl_addr=%s' % repr(self.nl_addr)]
        return 'netloc4(%s)' % ', '.join(out)
    __str__ = __repr__

class labelformat_spec4:
    # XDR definition:
    # struct labelformat_spec4 {
    #     policy4 lfs_lfs;
    #     policy4 lfs_pi;
    # };
    def __init__(self, lfs_lfs=None, lfs_pi=None):
        self.lfs_lfs = lfs_lfs
        self.lfs_pi = lfs_pi

    def __repr__(self):
        out = []
        if self.lfs_lfs is not None:
            out += ['lfs_lfs=%s' % repr(self.lfs_lfs)]
        if self.lfs_pi is not None:
            out += ['lfs_pi=%s' % repr(self.lfs_pi)]
        return 'labelformat_spec4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class sec_label4:
    ...
    #     opaque slai_data<>;
    # };
    def __init__(self, slai_lfs=None, slai_data=None):
        self.slai_lfs = slai_lfs
        self.slai_data = slai_data

    def __getattr__(self, attr):
        return getattr(self.slai_lfs, attr)

    def __repr__(self):
        out = []
        if self.slai_lfs is not None:
            out += ['slai_lfs=%s' % repr(self.slai_lfs)]
        if self.slai_data is not None:
            out += ['slai_data=%s' % repr(self.slai_data)]
        return 'sec_label4(%s)' % ', '.join(out)
    __str__ = __repr__

class copy_from_auth_priv:
    # XDR definition:
    ...
    # };
    def __init__(self, cfap_shared_secret=None, cfap_destination=None, cfap_username=None):
        self.cfap_shared_secret = cfap_shared_secret
        self.cfap_destination = cfap_destination
        self.cfap_username = cfap_username

    def __getattr__(self, attr):
        return getattr(self.cfap_destination, attr)

    def __repr__(self):
        out = []
        if self.cfap_shared_secret is not None:
            out += ['cfap_shared_secret=%s' % repr(self.cfap_shared_secret)]
        if self.cfap_destination is not None:
            out += ['cfap_destination=%s' % repr(self.cfap_destination)]
        if self.cfap_username is not None:
            out += ['cfap_username=%s' % repr(self.cfap_username)]
        return 'copy_from_auth_priv(%s)' % ', '.join(out)
    __str__ = __repr__

class copy_to_auth_priv:
    # XDR definition:
    ...
    # };
    def __init__(self, ctap_shared_secret=None, ctap_source=None, ctap_username=None):
        self.ctap_shared_secret = ctap_shared_secret
        self.ctap_source = ctap_source
        self.ctap_username = ctap_username

    def __repr__(self):
        out = []
        if self.ctap_shared_secret is not None:
            out += ['ctap_shared_secret=%s' % repr(self.ctap_shared_secret)]
        if self.ctap_source is not None:
            out += ['ctap_source=%s' % repr(self.ctap_source)]
        if self.ctap_username is not None:
            out += ['ctap_username=%s' % repr(self.ctap_username)]
        return 'copy_to_auth_priv(%s)' % ', '.join(out)
    __str__ = __repr__

class copy_confirm_auth_priv:
    # XDR definition:
    # struct copy_confirm_auth_priv {
    #     opaque ccap_shared_secret_mic<>;
    #     utf8str_mixed ccap_username;
    # };
    def __init__(self, ccap_shared_secret_mic=None, ccap_username=None):
        self.ccap_shared_secret_mic = ccap_shared_secret_mic
        self.ccap_username = ccap_username

    def __repr__(self):
        out = []
        if self.ccap_shared_secret_mic is not None:
            out += ['ccap_shared_secret_mic=%s' % repr(self.ccap_shared_secret_mic)]
        if self.ccap_username is not None:
            out += ['ccap_username=%s' % repr(self.ccap_username)]
        return 'copy_confirm_auth_priv(%s)' % ', '.join(out)
    __str__ = __repr__

class app_data_block4:
    # XDR definition:
    ...
    #     opaque adb_pattern<>;
    # };
    def __init__(self, adb_offset=None, adb_block_size=None, adb_block_count=None, adb_reloff_blocknum=None, adb_block_num=None, adb_reloff_pattern=None, adb_pattern=None):
        self.adb_offset = adb_offset
        self.adb_block_size = adb_block_size
        self.adb_block_count = adb_block_count
        self.adb_reloff_blocknum = adb_reloff_blocknum
        self.adb_block_num = adb_block_num
        self.adb_reloff_pattern = adb_reloff_pattern
        self.adb_pattern = adb_pattern

    def __repr__(self):
        out = []
        if self.adb_offset is not None:
            out += ['adb_offset=%s' % repr(self.adb_offset)]
        if self.adb_block_size is not None:
            out += ['adb_block_size=%s' % repr(self.adb_block_size)]
        if self.adb_block_count is not None:
            out += ['adb_block_count=%s' % repr(self.adb_block_count)]
        if self.adb_reloff_blocknum is not None:
            out += ['adb_reloff_blocknum=%s' % repr(self.adb_reloff_blocknum)]
        if self.adb_block_num is not None:
            out += ['adb_block_num=%s' % repr(self.adb_block_num)]
        if self.adb_reloff_pattern is not None:
            out += ['adb_reloff_pattern=%s' % repr(self.adb_reloff_pattern)]
        if self.adb_pattern is not None:
            out += ['adb_pattern=%s' % repr(self.adb_pattern)]
        return 'app_data_block4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class data4:
    ...
    #     opaque d_data<>;
    # };
    def __init__(self, d_offset=None, d_data=None):
    ...
    def __repr__(self):
        ...
        if self.d_offset is not None:
            out += ['d_offset=%s' % repr(self.d_offset)]
        if self.d_data is not None:
            out += ['d_data=%s' % repr(self.d_data)]
        return 'data4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class data_info4:
    ...
    #     length4 di_length;
    # };
    def __init__(self, di_offset=None, di_length=None):
        self.di_offset = di_offset
        self.di_length = di_length

    def __repr__(self):
        out = []
        if self.di_offset is not None:
            out += ['di_offset=%s' % repr(self.di_offset)]
        if self.di_length is not None:
            out += ['di_length=%s' % repr(self.di_length)]
        return 'data_info4(%s)' % ', '.join(out)
    __str__ = __repr__

class write_response4:
    # XDR definition:
    ...
    # };
    def __init__(self, wr_callback_id=None, wr_count=None, wr_committed=None, wr_writeverf=None):
        self.wr_callback_id = wr_callback_id
        self.wr_count = wr_count
        self.wr_committed = wr_committed
        self.wr_writeverf = wr_writeverf

    def __repr__(self):
        out = []
        if self.wr_callback_id is not None:
            out += ['wr_callback_id=%s' % repr(self.wr_callback_id)]
        if self.wr_count is not None:
            out += ['wr_count=%s' % repr(self.wr_count)]
        if self.wr_committed is not None:
            out += ['wr_committed=%s' % const.stable_how4.get(self.wr_committed, self.wr_committed)]
        if self.wr_writeverf is not None:
            out += ['wr_writeverf=%s' % repr(self.wr_writeverf)]
        return 'write_response4(%s)' % ', '.join(out)
    __str__ = __repr__
...
fattr4_change_attr_type = const.change_attr_type4
...
class CLONE4args:
    ...
    #     length4 cl_count;
    # };
    def __init__(self, cl_src_stateid=None, cl_dst_stateid=None, cl_src_offset=None, cl_dst_offset=None, cl_count=None):
        self.cl_src_stateid = cl_src_stateid
        self.cl_dst_stateid = cl_dst_stateid
        self.cl_src_offset = cl_src_offset
        self.cl_dst_offset = cl_dst_offset
        self.cl_count = cl_count

    def __repr__(self):
        out = []
        if self.cl_src_stateid is not None:
            out += ['cl_src_stateid=%s' % repr(self.cl_src_stateid)]
        if self.cl_dst_stateid is not None:
            out += ['cl_dst_stateid=%s' % repr(self.cl_dst_stateid)]
        if self.cl_src_offset is not None:
            out += ['cl_src_offset=%s' % repr(self.cl_src_offset)]
        if self.cl_dst_offset is not None:
            out += ['cl_dst_offset=%s' % repr(self.cl_dst_offset)]
        if self.cl_count is not None:
            out += ['cl_count=%s' % repr(self.cl_count)]
        return 'CLONE4args(%s)' % ', '.join(out)
    __str__ = __repr__
...
class CLONE4res:
    ...
    #     nfsstat4 cl_status;
    # };
    def __init__(self, cl_status=None):
        self.cl_status = cl_status

    def __repr__(self):
        out = []
        if self.cl_status is not None:
            out += ['cl_status=%s' % const.nfsstat4.get(self.cl_status, self.cl_status)]
        return 'CLONE4res(%s)' % ', '.join(out)
    __str__ = __repr__
...
class open_delegation4:
    ...
    switch = property(lambda s: {const.OPEN_DELEGATE_NONE:None,const.OPEN_DELEGATE_READ:s.read,const.OPEN_DELEGATE_READ_ATTRS_DELEG:s.read,const.OPEN_DELEGATE_WRITE:s.write,const.OPEN_DELEGATE_WRITE_ATTRS_DELEG:s.write,const.OPEN_DELEGATE_NONE_EXT:s.od_whynone,}[s.delegation_type])
...
class COPY4args:
    ...
    # };
    def __init__(self, ca_src_stateid=None, ca_dst_stateid=None, ca_src_offset=None, ca_dst_offset=None, ca_count=None, ca_consecutive=None, ca_synchronous=None, ca_source_server=None):
        self.ca_src_stateid = ca_src_stateid
        self.ca_dst_stateid = ca_dst_stateid
        self.ca_src_offset = ca_src_offset
        self.ca_dst_offset = ca_dst_offset
        self.ca_count = ca_count
        self.ca_consecutive = ca_consecutive
        self.ca_synchronous = ca_synchronous
        self.ca_source_server = ca_source_server

    def __repr__(self):
        out = []
        if self.ca_src_stateid is not None:
            out += ['ca_src_stateid=%s' % repr(self.ca_src_stateid)]
        if self.ca_dst_stateid is not None:
            out += ['ca_dst_stateid=%s' % repr(self.ca_dst_stateid)]
        if self.ca_src_offset is not None:
            out += ['ca_src_offset=%s' % repr(self.ca_src_offset)]
        if self.ca_dst_offset is not None:
            out += ['ca_dst_offset=%s' % repr(self.ca_dst_offset)]
        if self.ca_count is not None:
            out += ['ca_count=%s' % repr(self.ca_count)]
        if self.ca_consecutive is not None:
            out += ['ca_consecutive=%s' % repr(self.ca_consecutive)]
        if self.ca_synchronous is not None:
            out += ['ca_synchronous=%s' % repr(self.ca_synchronous)]
        if self.ca_source_server is not None:
            out += ['ca_source_server=%s' % repr(self.ca_source_server)]
        return 'COPY4args(%s)' % ', '.join(out)
    __str__ = __repr__

class copy_requirements4:
    # XDR definition:
    # struct copy_requirements4 {
    #     bool cr_consecutive;
    #     bool cr_synchronous;
    # };
    def __init__(self, cr_consecutive=None, cr_synchronous=None):
        self.cr_consecutive = cr_consecutive
        self.cr_synchronous = cr_synchronous

    def __repr__(self):
        out = []
        if self.cr_consecutive is not None:
            out += ['cr_consecutive=%s' % repr(self.cr_consecutive)]
        if self.cr_synchronous is not None:
            out += ['cr_synchronous=%s' % repr(self.cr_synchronous)]
        return 'copy_requirements4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class COPY4resok:
    ...
    # };
    def __init__(self, cr_response=None, cr_requirements=None):
        self.cr_response = cr_response
        self.cr_requirements = cr_requirements

    def __repr__(self):
        out = []
        if self.cr_response is not None:
            out += ['cr_response=%s' % repr(self.cr_response)]
        if self.cr_requirements is not None:
            out += ['cr_requirements=%s' % repr(self.cr_requirements)]
        return 'COPY4resok(%s)' % ', '.join(out)
    __str__ = __repr__
...
class COPY4res:
    ...
    #         void;
    # };
    def __init__(self, cr_status=None, cr_resok4=None, cr_requirements=None):
        self.cr_status = cr_status
        self.cr_resok4 = cr_resok4
        self.cr_requirements = cr_requirements

    switch = property(lambda s: {const.NFS4_OK:s.cr_resok4,const.NFS4ERR_OFFLOAD_NO_REQS:s.cr_requirements,}.get(s.cr_status, None))
    ...
    def __repr__(self):
        out = []
        if self.cr_status is not None:
            out += ['cr_status=%s' % const.nfsstat4.get(self.cr_status, self.cr_status)]
        if self.cr_resok4 is not None:
            out += ['cr_resok4=%s' % repr(self.cr_resok4)]
        if self.cr_requirements is not None:
            out += ['cr_requirements=%s' % repr(self.cr_requirements)]
        return 'COPY4res(%s)' % ', '.join(out)
    __str__ = __repr__

class COPY_NOTIFY4args:
    # XDR definition:
    # struct COPY_NOTIFY4args {
    #     stateid4 cna_src_stateid;
    #     netloc4 cna_destination_server;
    # };
    def __init__(self, cna_src_stateid=None, cna_destination_server=None):
        self.cna_src_stateid = cna_src_stateid
        self.cna_destination_server = cna_destination_server

    def __repr__(self):
        out = []
        if self.cna_src_stateid is not None:
            out += ['cna_src_stateid=%s' % repr(self.cna_src_stateid)]
        if self.cna_destination_server is not None:
            out += ['cna_destination_server=%s' % repr(self.cna_destination_server)]
        return 'COPY_NOTIFY4args(%s)' % ', '.join(out)
    __str__ = __repr__

class COPY_NOTIFY4resok:
    # XDR definition:
    ...
    # };
    def __init__(self, cnr_lease_time=None, cnr_stateid=None, cnr_source_server=None):
        self.cnr_lease_time = cnr_lease_time
        self.cnr_stateid = cnr_stateid
        self.cnr_source_server = cnr_source_server

    def __repr__(self):
        out = []
        if self.cnr_lease_time is not None:
            out += ['cnr_lease_time=%s' % repr(self.cnr_lease_time)]
        if self.cnr_stateid is not None:
            out += ['cnr_stateid=%s' % repr(self.cnr_stateid)]
        if self.cnr_source_server is not None:
            out += ['cnr_source_server=%s' % repr(self.cnr_source_server)]
        return 'COPY_NOTIFY4resok(%s)' % ', '.join(out)
    __str__ = __repr__

class COPY_NOTIFY4res:
    # XDR definition:
    ...
    #         void;
    # };
    def __init__(self, cnr_status=None, resok4=None):
        self.cnr_status = cnr_status
        self.resok4 = resok4

    switch = property(lambda s: {const.NFS4_OK:s.resok4,}.get(s.cnr_status, None))
    ...
    def __repr__(self):
        ...
        if self.resok4 is not None:
            out += ['resok4=%s' % repr(self.resok4)]
        return 'COPY_NOTIFY4res(%s)' % ', '.join(out)
    __str__ = __repr__

class OFFLOAD_CANCEL4args:
    # XDR definition:
    # struct OFFLOAD_CANCEL4args {
    #     stateid4 oca_stateid;
    # };
    def __init__(self, oca_stateid=None):
        self.oca_stateid = oca_stateid

    def __getattr__(self, attr):
        return getattr(self.oca_stateid, attr)

    def __repr__(self):
        out = []
        if self.oca_stateid is not None:
            out += ['oca_stateid=%s' % repr(self.oca_stateid)]
        return 'OFFLOAD_CANCEL4args(%s)' % ', '.join(out)
    __str__ = __repr__

class OFFLOAD_CANCEL4res:
    # XDR definition:
    # struct OFFLOAD_CANCEL4res {
    #     nfsstat4 ocr_status;
    # };
    def __init__(self, ocr_status=None):
        self.ocr_status = ocr_status

    def __repr__(self):
        out = []
        if self.ocr_status is not None:
            out += ['ocr_status=%s' % const.nfsstat4.get(self.ocr_status, self.ocr_status)]
        return 'OFFLOAD_CANCEL4res(%s)' % ', '.join(out)
    __str__ = __repr__

class OFFLOAD_STATUS4args:
    # XDR definition:
    # struct OFFLOAD_STATUS4args {
    #     stateid4 osa_stateid;
    # };
    def __init__(self, osa_stateid=None):
        self.osa_stateid = osa_stateid

    def __getattr__(self, attr):
        return getattr(self.osa_stateid, attr)

    def __repr__(self):
        out = []
        if self.osa_stateid is not None:
            out += ['osa_stateid=%s' % repr(self.osa_stateid)]
        return 'OFFLOAD_STATUS4args(%s)' % ', '.join(out)
    __str__ = __repr__

class OFFLOAD_STATUS4resok:
    # XDR definition:
    # struct OFFLOAD_STATUS4resok {
    #     length4 osr_count;
    #     nfsstat4 osr_complete<1>;
    # };
    def __init__(self, osr_count=None, osr_complete=None):
        self.osr_count = osr_count
        self.osr_complete = osr_complete

    def __repr__(self):
        out = []
        if self.osr_count is not None:
            out += ['osr_count=%s' % repr(self.osr_count)]
        if self.osr_complete is not None:
            out += ['osr_complete=%s' % ','.join([const.nfsstat4.get(x, x) for x in self.osr_complete])]
        return 'OFFLOAD_STATUS4resok(%s)' % ', '.join(out)
    __str__ = __repr__

class OFFLOAD_STATUS4res:
    # XDR definition:
    ...
    #         void;
    # };
    def __init__(self, osr_status=None, osr_resok4=None):
        self.osr_status = osr_status
        self.osr_resok4 = osr_resok4

    switch = property(lambda s: {const.NFS4_OK:s.osr_resok4,}.get(s.osr_status, None))
    ...
    def __repr__(self):
        out = []
        if self.osr_status is not None:
            out += ['osr_status=%s' % const.nfsstat4.get(self.osr_status, self.osr_status)]
        if self.osr_resok4 is not None:
            out += ['osr_resok4=%s' % repr(self.osr_resok4)]
        return 'OFFLOAD_STATUS4res(%s)' % ', '.join(out)
    __str__ = __repr__
...
class ALLOCATE4args:
    ...
    #     length4 aa_length;
    # };
    def __init__(self, aa_stateid=None, aa_offset=None, aa_length=None):
        self.aa_stateid = aa_stateid
        self.aa_offset = aa_offset
        self.aa_length = aa_length

    def __getattr__(self, attr):
        return getattr(self.aa_stateid, attr)

    def __repr__(self):
        out = []
        if self.aa_stateid is not None:
            out += ['aa_stateid=%s' % repr(self.aa_stateid)]
        if self.aa_offset is not None:
            out += ['aa_offset=%s' % repr(self.aa_offset)]
        if self.aa_length is not None:
            out += ['aa_length=%s' % repr(self.aa_length)]
        return 'ALLOCATE4args(%s)' % ', '.join(out)
    __str__ = __repr__
...
class ALLOCATE4res:
    ...
    #     nfsstat4 ar_status;
    # };
    def __init__(self, ar_status=None):
        self.ar_status = ar_status

    def __repr__(self):
        out = []
        if self.ar_status is not None:
            out += ['ar_status=%s' % const.nfsstat4.get(self.ar_status, self.ar_status)]
        return 'ALLOCATE4res(%s)' % ', '.join(out)
    __str__ = __repr__

class DEALLOCATE4args:
    # XDR definition:
    ...
    #     length4 da_length;
    # };
    def __init__(self, da_stateid=None, da_offset=None, da_length=None):
        self.da_stateid = da_stateid
        self.da_offset = da_offset
        self.da_length = da_length

    def __getattr__(self, attr):
        return getattr(self.da_stateid, attr)

    def __repr__(self):
        out = []
        if self.da_stateid is not None:
            out += ['da_stateid=%s' % repr(self.da_stateid)]
        if self.da_offset is not None:
            out += ['da_offset=%s' % repr(self.da_offset)]
        if self.da_length is not None:
            out += ['da_length=%s' % repr(self.da_length)]
        return 'DEALLOCATE4args(%s)' % ', '.join(out)
    __str__ = __repr__

class DEALLOCATE4res:
    # XDR definition:
    # struct DEALLOCATE4res {
    #     nfsstat4 dr_status;
    # };
    def __init__(self, dr_status=None):
        self.dr_status = dr_status

    def __repr__(self):
        out = []
        if self.dr_status is not None:
            out += ['dr_status=%s' % const.nfsstat4.get(self.dr_status, self.dr_status)]
        return 'DEALLOCATE4res(%s)' % ', '.join(out)
    __str__ = __repr__
...
class IO_ADVISE4args:
    ...
    #     bitmap4 iaa_hints;
    # };
    def __init__(self, iaa_stateid=None, iaa_offset=None, iaa_count=None, iaa_hints=None):
        self.iaa_stateid = iaa_stateid
        self.iaa_offset = iaa_offset
        self.iaa_count = iaa_count
        self.iaa_hints = iaa_hints

    def __getattr__(self, attr):
        return getattr(self.iaa_stateid, attr)

    def __repr__(self):
        out = []
        if self.iaa_stateid is not None:
            out += ['iaa_stateid=%s' % repr(self.iaa_stateid)]
        if self.iaa_offset is not None:
            out += ['iaa_offset=%s' % repr(self.iaa_offset)]
        if self.iaa_count is not None:
            out += ['iaa_count=%s' % repr(self.iaa_count)]
        if self.iaa_hints is not None:
            out += ['iaa_hints=%s' % repr(self.iaa_hints)]
        return 'IO_ADVISE4args(%s)' % ', '.join(out)
    __str__ = __repr__

class IO_ADVISE4resok:
    # XDR definition:
    # struct IO_ADVISE4resok {
    #     bitmap4 ior_hints;
    # };
    def __init__(self, ior_hints=None):
        self.ior_hints = ior_hints

    def __repr__(self):
        out = []
        if self.ior_hints is not None:
            out += ['ior_hints=%s' % repr(self.ior_hints)]
        return 'IO_ADVISE4resok(%s)' % ', '.join(out)
    __str__ = __repr__
...
class IO_ADVISE4res:
    ...
    #         void;
    # };
    def __init__(self, ior_status=None, resok4=None):
        self.ior_status = ior_status
        self.resok4 = resok4

    switch = property(lambda s: {const.NFS4_OK:s.resok4,}.get(s.ior_status, None))
    ...
    def __repr__(self):
        out = []
        if self.ior_status is not None:
            out += ['ior_status=%s' % const.nfsstat4.get(self.ior_status, self.ior_status)]
        if self.resok4 is not None:
            out += ['resok4=%s' % repr(self.resok4)]
        return 'IO_ADVISE4res(%s)' % ', '.join(out)
    __str__ = __repr__
...
class device_error4:
    ...
    #     nfs_opnum4 de_opnum;
    # };
    def __init__(self, de_deviceid=None, de_status=None, de_opnum=None):
        self.de_deviceid = de_deviceid
        self.de_status = de_status
        self.de_opnum = de_opnum

    def __repr__(self):
        out = []
        if self.de_deviceid is not None:
            out += ['de_deviceid=%s' % repr(self.de_deviceid)]
        if self.de_status is not None:
            out += ['de_status=%s' % const.nfsstat4.get(self.de_status, self.de_status)]
        if self.de_opnum is not None:
            out += ['de_opnum=%s' % const.nfs_opnum4.get(self.de_opnum, self.de_opnum)]
        return 'device_error4(%s)' % ', '.join(out)
    __str__ = __repr__

class LAYOUTERROR4args:
    # XDR definition:
    ...
    # };
    def __init__(self, lea_offset=None, lea_length=None, lea_stateid=None, lea_errors=None):
        self.lea_offset = lea_offset
        self.lea_length = lea_length
        self.lea_stateid = lea_stateid
        self.lea_errors = lea_errors

    def __getattr__(self, attr):
        return getattr(self.lea_stateid, attr)

    def __repr__(self):
        out = []
        if self.lea_offset is not None:
            out += ['lea_offset=%s' % repr(self.lea_offset)]
        if self.lea_length is not None:
            out += ['lea_length=%s' % repr(self.lea_length)]
        if self.lea_stateid is not None:
            out += ['lea_stateid=%s' % repr(self.lea_stateid)]
        if self.lea_errors is not None:
            out += ['lea_errors=%s' % repr(self.lea_errors)]
        return 'LAYOUTERROR4args(%s)' % ', '.join(out)
    __str__ = __repr__

class LAYOUTERROR4res:
    # XDR definition:
    # struct LAYOUTERROR4res {
    #     nfsstat4 ler_status;
    # };
    def __init__(self, ler_status=None):
        self.ler_status = ler_status

    def __repr__(self):
        out = []
        if self.ler_status is not None:
            out += ['ler_status=%s' % const.nfsstat4.get(self.ler_status, self.ler_status)]
        return 'LAYOUTERROR4res(%s)' % ', '.join(out)
    __str__ = __repr__
...
class io_info4:
    ...
    #     uint64_t ii_bytes;
    # };
    def __init__(self, ii_count=None, ii_bytes=None):
        self.ii_count = ii_count
        self.ii_bytes = ii_bytes

    def __repr__(self):
        out = []
        if self.ii_count is not None:
            out += ['ii_count=%s' % repr(self.ii_count)]
        if self.ii_bytes is not None:
            out += ['ii_bytes=%s' % repr(self.ii_bytes)]
        return 'io_info4(%s)' % ', '.join(out)
    __str__ = __repr__

class LAYOUTSTATS4args:
    # XDR definition:
    ...
    # };
    def __init__(self, lsa_offset=None, lsa_length=None, lsa_stateid=None, lsa_read=None, lsa_write=None, lsa_deviceid=None, lsa_layoutupdate=None):
        self.lsa_offset = lsa_offset
        self.lsa_length = lsa_length
        self.lsa_stateid = lsa_stateid
        self.lsa_read = lsa_read
        self.lsa_write = lsa_write
        self.lsa_deviceid = lsa_deviceid
        self.lsa_layoutupdate = lsa_layoutupdate

    def __repr__(self):
        out = []
        if self.lsa_offset is not None:
            out += ['lsa_offset=%s' % repr(self.lsa_offset)]
        if self.lsa_length is not None:
            out += ['lsa_length=%s' % repr(self.lsa_length)]
        if self.lsa_stateid is not None:
            out += ['lsa_stateid=%s' % repr(self.lsa_stateid)]
        if self.lsa_read is not None:
            out += ['lsa_read=%s' % repr(self.lsa_read)]
        if self.lsa_write is not None:
            out += ['lsa_write=%s' % repr(self.lsa_write)]
        if self.lsa_deviceid is not None:
            out += ['lsa_deviceid=%s' % repr(self.lsa_deviceid)]
        if self.lsa_layoutupdate is not None:
            out += ['lsa_layoutupdate=%s' % repr(self.lsa_layoutupdate)]
        return 'LAYOUTSTATS4args(%s)' % ', '.join(out)
    __str__ = __repr__

class LAYOUTSTATS4res:
    # XDR definition:
    # struct LAYOUTSTATS4res {
    #     nfsstat4 lsr_status;
    # };
    def __init__(self, lsr_status=None):
        self.lsr_status = lsr_status

    def __repr__(self):
        out = []
        if self.lsr_status is not None:
            out += ['lsr_status=%s' % const.nfsstat4.get(self.lsr_status, self.lsr_status)]
        return 'LAYOUTSTATS4res(%s)' % ', '.join(out)
    __str__ = __repr__

class READ_PLUS4args:
    # XDR definition:
    ...
    #     count4 rpa_count;
    # };
    def __init__(self, rpa_stateid=None, rpa_offset=None, rpa_count=None):
        self.rpa_stateid = rpa_stateid
        self.rpa_offset = rpa_offset
        self.rpa_count = rpa_count

    def __getattr__(self, attr):
        return getattr(self.rpa_stateid, attr)

    def __repr__(self):
        out = []
        if self.rpa_stateid is not None:
            out += ['rpa_stateid=%s' % repr(self.rpa_stateid)]
        if self.rpa_offset is not None:
            out += ['rpa_offset=%s' % repr(self.rpa_offset)]
        if self.rpa_count is not None:
            out += ['rpa_count=%s' % repr(self.rpa_count)]
        return 'READ_PLUS4args(%s)' % ', '.join(out)
    __str__ = __repr__

class read_plus_content:
    # XDR definition:
    ...
    #         void;
    # };
    def __init__(self, rpc_content=None, rpc_data=None, rpc_hole=None):
        self.rpc_content = rpc_content
        self.rpc_data = rpc_data
        self.rpc_hole = rpc_hole

    switch = property(lambda s: {const.NFS4_CONTENT_DATA:s.rpc_data,const.NFS4_CONTENT_HOLE:s.rpc_hole,}.get(s.rpc_content, None))
    ...
    def __repr__(self):
        out = []
        if self.rpc_content is not None:
            out += ['rpc_content=%s' % const.data_content4.get(self.rpc_content, self.rpc_content)]
        if self.rpc_data is not None:
            out += ['rpc_data=%s' % repr(self.rpc_data)]
        if self.rpc_hole is not None:
            out += ['rpc_hole=%s' % repr(self.rpc_hole)]
        return 'read_plus_content(%s)' % ', '.join(out)
    __str__ = __repr__

class read_plus_res4:
    # XDR definition:
    # struct read_plus_res4 {
    #     bool rpr_eof;
    #     read_plus_content rpr_contents<>;
    # };
    def __init__(self, rpr_eof=None, rpr_contents=None):
        self.rpr_eof = rpr_eof
        self.rpr_contents = rpr_contents

    def __repr__(self):
        out = []
        if self.rpr_eof is not None:
            out += ['rpr_eof=%s' % repr(self.rpr_eof)]
        if self.rpr_contents is not None:
            out += ['rpr_contents=%s' % repr(self.rpr_contents)]
        return 'read_plus_res4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class READ_PLUS4res:
    ...
    #         void;
    # };
    def __init__(self, rp_status=None, rp_resok4=None):
        self.rp_status = rp_status
        self.rp_resok4 = rp_resok4

    switch = property(lambda s: {const.NFS4_OK:s.rp_resok4,}.get(s.rp_status, None))
    ...
    def __repr__(self):
        ...
        if self.rp_status is not None:
            out += ['rp_status=%s' % const.nfsstat4.get(self.rp_status, self.rp_status)]
        if self.rp_resok4 is not None:
            out += ['rp_resok4=%s' % repr(self.rp_resok4)]
        return 'READ_PLUS4res(%s)' % ', '.join(out)
    __str__ = __repr__
...
class SEEK4args:
    ...
    # };
    def __init__(self, sa_stateid=None, sa_offset=None, sa_what=None):
        self.sa_stateid = sa_stateid
        self.sa_offset = sa_offset
        self.sa_what = sa_what

    def __getattr__(self, attr):
        return getattr(self.sa_stateid, attr)

    def __repr__(self):
        out = []
        if self.sa_stateid is not None:
            out += ['sa_stateid=%s' % repr(self.sa_stateid)]
        if self.sa_offset is not None:
            out += ['sa_offset=%s' % repr(self.sa_offset)]
        if self.sa_what is not None:
            out += ['sa_what=%s' % const.data_content4.get(self.sa_what, self.sa_what)]
        return 'SEEK4args(%s)' % ', '.join(out)
    __str__ = __repr__
...
class seek_res4:
    ...
    #     offset4 sr_offset;
    # };
    def __init__(self, sr_eof=None, sr_offset=None):
        self.sr_eof = sr_eof
        self.sr_offset = sr_offset

    def __repr__(self):
        out = []
        if self.sr_eof is not None:
            out += ['sr_eof=%s' % repr(self.sr_eof)]
        if self.sr_offset is not None:
            out += ['sr_offset=%s' % repr(self.sr_offset)]
        return 'seek_res4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class SEEK4res:
    ...
    #         void;
    # };
    def __init__(self, sa_status=None, resok4=None):
        self.sa_status = sa_status
        self.resok4 = resok4

    switch = property(lambda s: {const.NFS4_OK:s.resok4,}.get(s.sa_status, None))
    ...
    def __repr__(self):
        out = []
        if self.sa_status is not None:
            out += ['sa_status=%s' % const.nfsstat4.get(self.sa_status, self.sa_status)]
        if self.resok4 is not None:
            out += ['resok4=%s' % repr(self.resok4)]
        return 'SEEK4res(%s)' % ', '.join(out)
    __str__ = __repr__

class WRITE_SAME4args:
    # XDR definition:
    ...
    # };
    def __init__(self, wsa_stateid=None, wsa_stable=None, wsa_adb=None):
        self.wsa_stateid = wsa_stateid
        self.wsa_stable = wsa_stable
        self.wsa_adb = wsa_adb

    def __repr__(self):
        out = []
        if self.wsa_stateid is not None:
            out += ['wsa_stateid=%s' % repr(self.wsa_stateid)]
        if self.wsa_stable is not None:
            out += ['wsa_stable=%s' % const.stable_how4.get(self.wsa_stable, self.wsa_stable)]
        if self.wsa_adb is not None:
            out += ['wsa_adb=%s' % repr(self.wsa_adb)]
        return 'WRITE_SAME4args(%s)' % ', '.join(out)
    __str__ = __repr__

class WRITE_SAME4res:
    # XDR definition:
    ...
    #         void;
    # };
    def __init__(self, wsr_status=None, resok4=None):
        self.wsr_status = wsr_status
        self.resok4 = resok4

    switch = property(lambda s: {const.NFS4_OK:s.resok4,}.get(s.wsr_status, None))
    ...
    def __repr__(self):
        out = []
        if self.wsr_status is not None:
            out += ['wsr_status=%s' % const.nfsstat4.get(self.wsr_status, self.wsr_status)]
        if self.resok4 is not None:
            out += ['resok4=%s' % repr(self.resok4)]
        return 'WRITE_SAME4res(%s)' % ', '.join(out)
    __str__ = __repr__

class ff_device_versions4:
    # XDR definition:
    ...
    # };
    def __init__(self, ffdv_version=None, ffdv_minorversion=None, ffdv_rsize=None, ffdv_wsize=None, ffdv_tightly_coupled=None):
        self.ffdv_version = ffdv_version
        self.ffdv_minorversion = ffdv_minorversion
        self.ffdv_rsize = ffdv_rsize
        self.ffdv_wsize = ffdv_wsize
        self.ffdv_tightly_coupled = ffdv_tightly_coupled

    def __repr__(self):
        out = []
        if self.ffdv_version is not None:
            out += ['ffdv_version=%s' % repr(self.ffdv_version)]
        if self.ffdv_minorversion is not None:
            out += ['ffdv_minorversion=%s' % repr(self.ffdv_minorversion)]
        if self.ffdv_rsize is not None:
            out += ['ffdv_rsize=%s' % repr(self.ffdv_rsize)]
        if self.ffdv_wsize is not None:
            out += ['ffdv_wsize=%s' % repr(self.ffdv_wsize)]
        if self.ffdv_tightly_coupled is not None:
            out += ['ffdv_tightly_coupled=%s' % repr(self.ffdv_tightly_coupled)]
        return 'ff_device_versions4(%s)' % ', '.join(out)
    __str__ = __repr__

class ff_device_addr4:
    # XDR definition:
    # struct ff_device_addr4 {
    #     multipath_list4 ffda_netaddrs;
    #     ff_device_versions4 ffda_versions<>;
    # };
    def __init__(self, ffda_netaddrs=None, ffda_versions=None):
        self.ffda_netaddrs = ffda_netaddrs
        self.ffda_versions = ffda_versions

    def __repr__(self):
        out = []
        if self.ffda_netaddrs is not None:
            out += ['ffda_netaddrs=%s' % repr(self.ffda_netaddrs)]
        if self.ffda_versions is not None:
            out += ['ffda_versions=%s' % repr(self.ffda_versions)]
        return 'ff_device_addr4(%s)' % ', '.join(out)
    __str__ = __repr__

class ff_data_server4:
    # XDR definition:
    ...
    # };
    def __init__(self, ffds_deviceid=None, ffds_efficiency=None, ffds_stateid=None, ffds_fh_vers=None, ffds_user=None, ffds_group=None):
        self.ffds_deviceid = ffds_deviceid
        self.ffds_efficiency = ffds_efficiency
        self.ffds_stateid = ffds_stateid
        self.ffds_fh_vers = ffds_fh_vers
        self.ffds_user = ffds_user
        self.ffds_group = ffds_group

    def __getattr__(self, attr):
        return getattr(self.ffds_stateid, attr)

    def __repr__(self):
        out = []
        if self.ffds_deviceid is not None:
            out += ['ffds_deviceid=%s' % repr(self.ffds_deviceid)]
        if self.ffds_efficiency is not None:
            out += ['ffds_efficiency=%s' % repr(self.ffds_efficiency)]
        if self.ffds_stateid is not None:
            out += ['ffds_stateid=%s' % repr(self.ffds_stateid)]
        if self.ffds_fh_vers is not None:
            out += ['ffds_fh_vers=%s' % repr(self.ffds_fh_vers)]
        if self.ffds_user is not None:
            out += ['ffds_user=%s' % repr(self.ffds_user)]
        if self.ffds_group is not None:
            out += ['ffds_group=%s' % repr(self.ffds_group)]
        return 'ff_data_server4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class ff_mirror4:
    ...
    # };
    def __init__(self, ffm_data_servers=None):
        self.ffm_data_servers = ffm_data_servers

    def __repr__(self):
        out = []
        if self.ffm_data_servers is not None:
            out += ['ffm_data_servers=%s' % repr(self.ffm_data_servers)]
        return 'ff_mirror4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class ff_layout4:
    ...
    # };
    def __init__(self, ffl_stripe_unit=None, ffl_mirrors=None, ffl_flags=None, ffl_stats_collect_hint=None):
        self.ffl_stripe_unit = ffl_stripe_unit
        self.ffl_mirrors = ffl_mirrors
        self.ffl_flags = ffl_flags
        self.ffl_stats_collect_hint = ffl_stats_collect_hint

    def __repr__(self):
        out = []
        if self.ffl_stripe_unit is not None:
            out += ['ffl_stripe_unit=%s' % repr(self.ffl_stripe_unit)]
        if self.ffl_mirrors is not None:
            out += ['ffl_mirrors=%s' % repr(self.ffl_mirrors)]
        if self.ffl_flags is not None:
            out += ['ffl_flags=%s' % repr(self.ffl_flags)]
        if self.ffl_stats_collect_hint is not None:
            out += ['ffl_stats_collect_hint=%s' % repr(self.ffl_stats_collect_hint)]
        return 'ff_layout4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class ff_ioerr4:
    ...
    # };
    def __init__(self, ffie_offset=None, ffie_length=None, ffie_stateid=None, ffie_errors=None):
        self.ffie_offset = ffie_offset
        self.ffie_length = ffie_length
        self.ffie_stateid = ffie_stateid
        self.ffie_errors = ffie_errors

    def __getattr__(self, attr):
        return getattr(self.ffie_stateid, attr)

    def __repr__(self):
        out = []
        if self.ffie_offset is not None:
            out += ['ffie_offset=%s' % repr(self.ffie_offset)]
        if self.ffie_length is not None:
            out += ['ffie_length=%s' % repr(self.ffie_length)]
        if self.ffie_stateid is not None:
            out += ['ffie_stateid=%s' % repr(self.ffie_stateid)]
        if self.ffie_errors is not None:
            out += ['ffie_errors=%s' % repr(self.ffie_errors)]
        return 'ff_ioerr4(%s)' % ', '.join(out)
    __str__ = __repr__

class ff_io_latency4:
    # XDR definition:
    ...
    # };
    def __init__(self, ffil_ops_requested=None, ffil_bytes_requested=None, ffil_ops_completed=None, ffil_bytes_completed=None, ffil_bytes_not_delivered=None, ffil_total_busy_time=None, ffil_aggregate_completion_time=None):
        self.ffil_ops_requested = ffil_ops_requested
        self.ffil_bytes_requested = ffil_bytes_requested
        self.ffil_ops_completed = ffil_ops_completed
        self.ffil_bytes_completed = ffil_bytes_completed
        self.ffil_bytes_not_delivered = ffil_bytes_not_delivered
        self.ffil_total_busy_time = ffil_total_busy_time
        self.ffil_aggregate_completion_time = ffil_aggregate_completion_time

    def __repr__(self):
        out = []
        if self.ffil_ops_requested is not None:
            out += ['ffil_ops_requested=%s' % repr(self.ffil_ops_requested)]
        if self.ffil_bytes_requested is not None:
            out += ['ffil_bytes_requested=%s' % repr(self.ffil_bytes_requested)]
        if self.ffil_ops_completed is not None:
            out += ['ffil_ops_completed=%s' % repr(self.ffil_ops_completed)]
        if self.ffil_bytes_completed is not None:
            out += ['ffil_bytes_completed=%s' % repr(self.ffil_bytes_completed)]
        if self.ffil_bytes_not_delivered is not None:
            out += ['ffil_bytes_not_delivered=%s' % repr(self.ffil_bytes_not_delivered)]
        if self.ffil_total_busy_time is not None:
            out += ['ffil_total_busy_time=%s' % repr(self.ffil_total_busy_time)]
        if self.ffil_aggregate_completion_time is not None:
            out += ['ffil_aggregate_completion_time=%s' % repr(self.ffil_aggregate_completion_time)]
        return 'ff_io_latency4(%s)' % ', '.join(out)
    __str__ = __repr__

class ff_layoutupdate4:
    # XDR definition:
    ...
    #     bool ffl_local;
    # };
    def __init__(self, ffl_addr=None, ffl_fhandle=None, ffl_read=None, ffl_write=None, ffl_duration=None, ffl_local=None):
        self.ffl_addr = ffl_addr
        self.ffl_fhandle = ffl_fhandle
        self.ffl_read = ffl_read
        self.ffl_write = ffl_write
        self.ffl_duration = ffl_duration
        self.ffl_local = ffl_local

    def __repr__(self):
        out = []
        if self.ffl_addr is not None:
            out += ['ffl_addr=%s' % repr(self.ffl_addr)]
        if self.ffl_fhandle is not None:
            out += ['ffl_fhandle=%s' % repr(self.ffl_fhandle)]
        if self.ffl_read is not None:
            out += ['ffl_read=%s' % repr(self.ffl_read)]
        if self.ffl_write is not None:
            out += ['ffl_write=%s' % repr(self.ffl_write)]
        if self.ffl_duration is not None:
            out += ['ffl_duration=%s' % repr(self.ffl_duration)]
        if self.ffl_local is not None:
            out += ['ffl_local=%s' % repr(self.ffl_local)]
        return 'ff_layoutupdate4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class ff_iostats4:
    ...
    # };
    def __init__(self, ffis_offset=None, ffis_length=None, ffis_stateid=None, ffis_read=None, ffis_write=None, ffis_deviceid=None, ffis_layoutupdate=None):
        self.ffis_offset = ffis_offset
        self.ffis_length = ffis_length
        self.ffis_stateid = ffis_stateid
        self.ffis_read = ffis_read
        self.ffis_write = ffis_write
        self.ffis_deviceid = ffis_deviceid
        self.ffis_layoutupdate = ffis_layoutupdate

    def __repr__(self):
        out = []
        if self.ffis_offset is not None:
            out += ['ffis_offset=%s' % repr(self.ffis_offset)]
        if self.ffis_length is not None:
            out += ['ffis_length=%s' % repr(self.ffis_length)]
        if self.ffis_stateid is not None:
            out += ['ffis_stateid=%s' % repr(self.ffis_stateid)]
        if self.ffis_read is not None:
            out += ['ffis_read=%s' % repr(self.ffis_read)]
        if self.ffis_write is not None:
            out += ['ffis_write=%s' % repr(self.ffis_write)]
        if self.ffis_deviceid is not None:
            out += ['ffis_deviceid=%s' % repr(self.ffis_deviceid)]
        if self.ffis_layoutupdate is not None:
            out += ['ffis_layoutupdate=%s' % repr(self.ffis_layoutupdate)]
        return 'ff_iostats4(%s)' % ', '.join(out)
    __str__ = __repr__

class ff_layoutreturn4:
    # XDR definition:
    # struct ff_layoutreturn4 {
    #     ff_ioerr4 fflr_ioerr_report<>;
    #     ff_iostats4 fflr_iostats_report<>;
    # };
    def __init__(self, fflr_ioerr_report=None, fflr_iostats_report=None):
        self.fflr_ioerr_report = fflr_ioerr_report
        self.fflr_iostats_report = fflr_iostats_report

    def __repr__(self):
        out = []
        if self.fflr_ioerr_report is not None:
            out += ['fflr_ioerr_report=%s' % repr(self.fflr_ioerr_report)]
        if self.fflr_iostats_report is not None:
            out += ['fflr_iostats_report=%s' % repr(self.fflr_iostats_report)]
        return 'ff_layoutreturn4(%s)' % ', '.join(out)
    __str__ = __repr__

class ff_mirrors_hint:
    # XDR definition:
    ...
    #         void;
    # };
    def __init__(self, ffmc_valid=None, ffmc_mirrors=None):
        self.ffmc_valid = ffmc_valid
        self.ffmc_mirrors = ffmc_mirrors

    switch = property(lambda s: {const.TRUE:s.ffmc_mirrors,const.FALSE:None,}[s.ffmc_valid])
    ...
    def __repr__(self):
        out = []
        if self.ffmc_valid is not None:
            out += ['ffmc_valid=%s' % repr(self.ffmc_valid)]
        if self.ffmc_mirrors is not None:
            out += ['ffmc_mirrors=%s' % repr(self.ffmc_mirrors)]
        return 'ff_mirrors_hint(%s)' % ', '.join(out)
    __str__ = __repr__

class ff_layouthint4:
    # XDR definition:
    # struct ff_layouthint4 {
    #     ff_mirrors_hint fflh_mirrors_hint;
    # };
    def __init__(self, fflh_mirrors_hint=None):
        self.fflh_mirrors_hint = fflh_mirrors_hint

    def __getattr__(self, attr):
        return getattr(self.fflh_mirrors_hint, attr)

    def __repr__(self):
        out = []
        if self.fflh_mirrors_hint is not None:
            out += ['fflh_mirrors_hint=%s' % repr(self.fflh_mirrors_hint)]
        return 'ff_layouthint4(%s)' % ', '.join(out)
    __str__ = __repr__
...
class GETXATTR4args:
    ...
    #     xattrkey4 gxa_name;
    # };
    def __init__(self, gxa_name=None):
        self.gxa_name = gxa_name

    def __repr__(self):
        out = []
        if self.gxa_name is not None:
            out += ['gxa_name=%s' % repr(self.gxa_name)]
        return 'GETXATTR4args(%s)' % ', '.join(out)
    __str__ = __repr__
...
class GETXATTR4res:
    ...
    #         void;
    # };
    def __init__(self, gxr_status=None, gxr_value=None):
        self.gxr_status = gxr_status
        self.gxr_value = gxr_value

    switch = property(lambda s: {const.NFS4_OK:s.gxr_value,}.get(s.gxr_status, None))
    ...
    def __repr__(self):
        out = []
        if self.gxr_status is not None:
            out += ['gxr_status=%s' % const.nfsstat4.get(self.gxr_status, self.gxr_status)]
        if self.gxr_value is not None:
            out += ['gxr_value=%s' % repr(self.gxr_value)]
        return 'GETXATTR4res(%s)' % ', '.join(out)
    __str__ = __repr__
...
class SETXATTR4args:
    ...
    # };
    def __init__(self, sxa_option=None, sxa_key=None, sxa_value=None):
        self.sxa_option = sxa_option
        self.sxa_key = sxa_key
        self.sxa_value = sxa_value

    def __repr__(self):
        out = []
        if self.sxa_option is not None:
            out += ['sxa_option=%s' % const.setxattr_option4.get(self.sxa_option, self.sxa_option)]
        if self.sxa_key is not None:
            out += ['sxa_key=%s' % repr(self.sxa_key)]
        if self.sxa_value is not None:
            out += ['sxa_value=%s' % repr(self.sxa_value)]
        return 'SETXATTR4args(%s)' % ', '.join(out)
    __str__ = __repr__
...
class SETXATTR4res:
    ...
    #         void;
    # };
    def __init__(self, sxr_status=None, sxr_info=None):
        self.sxr_status = sxr_status
        self.sxr_info = sxr_info

    switch = property(lambda s: {const.NFS4_OK:s.sxr_info,}.get(s.sxr_status, None))
    ...
    def __repr__(self):
        out = []
        if self.sxr_status is not None:
            out += ['sxr_status=%s' % const.nfsstat4.get(self.sxr_status, self.sxr_status)]
        if self.sxr_info is not None:
            out += ['sxr_info=%s' % repr(self.sxr_info)]
        return 'SETXATTR4res(%s)' % ', '.join(out)
    __str__ = __repr__

class LISTXATTRS4args:
    # XDR definition:
    # struct LISTXATTRS4args {
    #     nfs_cookie4 lxa_cookie;
    #     count4 lxa_maxcount;
    # };
    def __init__(self, lxa_cookie=None, lxa_maxcount=None):
        self.lxa_cookie = lxa_cookie
        self.lxa_maxcount = lxa_maxcount

    def __repr__(self):
        out = []
        if self.lxa_cookie is not None:
            out += ['lxa_cookie=%s' % repr(self.lxa_cookie)]
        if self.lxa_maxcount is not None:
            out += ['lxa_maxcount=%s' % repr(self.lxa_maxcount)]
        return 'LISTXATTRS4args(%s)' % ', '.join(out)
    __str__ = __repr__

class LISTXATTRS4resok:
    # XDR definition:
    ...
    #     bool lxr_eof;
    # };
    def __init__(self, lxr_cookie=None, lxr_names=None, lxr_eof=None):
        self.lxr_cookie = lxr_cookie
        self.lxr_names = lxr_names
        self.lxr_eof = lxr_eof

    def __repr__(self):
        out = []
        if self.lxr_cookie is not None:
            out += ['lxr_cookie=%s' % repr(self.lxr_cookie)]
        if self.lxr_names is not None:
            out += ['lxr_names=%s' % repr(self.lxr_names)]
        if self.lxr_eof is not None:
            out += ['lxr_eof=%s' % repr(self.lxr_eof)]
        return 'LISTXATTRS4resok(%s)' % ', '.join(out)
    __str__ = __repr__

class LISTXATTRS4res:
    # XDR definition:
    ...
    #         void;
    # };
    def __init__(self, lxr_status=None, lxr_value=None):
        self.lxr_status = lxr_status
        self.lxr_value = lxr_value

    switch = property(lambda s: {const.NFS4_OK:s.lxr_value,}.get(s.lxr_status, None))
    ...
    def __repr__(self):
        out = []
        if self.lxr_status is not None:
            out += ['lxr_status=%s' % const.nfsstat4.get(self.lxr_status, self.lxr_status)]
        if self.lxr_value is not None:
            out += ['lxr_value=%s' % repr(self.lxr_value)]
        return 'LISTXATTRS4res(%s)' % ', '.join(out)
    __str__ = __repr__

class REMOVEXATTR4args:
    # XDR definition:
    # struct REMOVEXATTR4args {
    #     xattrkey4 rxa_name;
    # };
    def __init__(self, rxa_name=None):
        self.rxa_name = rxa_name

    def __repr__(self):
        out = []
        if self.rxa_name is not None:
            out += ['rxa_name=%s' % repr(self.rxa_name)]
        return 'REMOVEXATTR4args(%s)' % ', '.join(out)
    __str__ = __repr__

class REMOVEXATTR4res:
    # XDR definition:
    ...
    #         void;
    # };
    def __init__(self, rxr_status=None, rxr_info=None):
        self.rxr_status = rxr_status
        self.rxr_info = rxr_info

    switch = property(lambda s: {const.NFS4_OK:s.rxr_info,}.get(s.rxr_status, None))
    ...
    def __repr__(self):
        out = []
        if self.rxr_status is not None:
            out += ['rxr_status=%s' % const.nfsstat4.get(self.rxr_status, self.rxr_status)]
        if self.rxr_info is not None:
            out += ['rxr_info=%s' % repr(self.rxr_info)]
        return 'REMOVEXATTR4res(%s)' % ', '.join(out)
    __str__ = __repr__
...
class nfs_argop4:
    ...
    #         void;
    # };
    def __init__(self, argop=None, opaccess=None, opclose=None, opcommit=None, opcreate=None, opdelegpurge=None, opdelegreturn=None, opgetattr=None, oplink=None, oplock=None, oplockt=None, oplocku=None, oplookup=None, opnverify=None, opopen=None, opopenattr=None, opopen_confirm=None, opopen_downgrade=None, opputfh=None, opread=None, opreaddir=None, opremove=None, oprename=None, oprenew=None, opsecinfo=None, opsetattr=None, opsetclientid=None, opsetclientid_confirm=None, opverify=None, opwrite=None, oprelease_lockowner=None, opbackchannel_ctl=None, opbind_conn_to_session=None, opexchange_id=None, opcreate_session=None, opdestroy_session=None, opfree_stateid=None, opget_dir_delegation=None, opgetdeviceinfo=None, opgetdevicelist=None, oplayoutcommit=None, oplayoutget=None, oplayoutreturn=None, opsecinfo_no_name=None, opsequence=None, opset_ssv=None, optest_stateid=None, opwant_delegation=None, opdestroy_clientid=None, opreclaim_complete=None, opallocate=None, opcopy=None, opoffload_notify=None, opdeallocate=None, opio_advise=None, oplayouterror=None, oplayoutstats=None, opoffload_cancel=None, opoffload_status=None, opread_plus=None, opseek=None, opwrite_same=None, opclone=None, opgetxattr=None, opsetxattr=None, oplistxattrs=None, opremovexattr=None):
        self.argop = argop
    ...
    def __repr__(self):
        ...
        if self.opallocate is not None:
            out += ['opallocate=%s' % repr(self.opallocate)]
        if self.opcopy is not None:
            out += ['opcopy=%s' % repr(self.opcopy)]
        if self.opoffload_notify is not None:
            out += ['opoffload_notify=%s' % repr(self.opoffload_notify)]
        if self.opdeallocate is not None:
            out += ['opdeallocate=%s' % repr(self.opdeallocate)]
        if self.opio_advise is not None:
            out += ['opio_advise=%s' % repr(self.opio_advise)]
        if self.oplayouterror is not None:
            out += ['oplayouterror=%s' % repr(self.oplayouterror)]
        if self.oplayoutstats is not None:
            out += ['oplayoutstats=%s' % repr(self.oplayoutstats)]
        if self.opoffload_cancel is not None:
            out += ['opoffload_cancel=%s' % repr(self.opoffload_cancel)]
        if self.opoffload_status is not None:
            out += ['opoffload_status=%s' % repr(self.opoffload_status)]
        if self.opread_plus is not None:
            out += ['opread_plus=%s' % repr(self.opread_plus)]
        if self.opseek is not None:
            out += ['opseek=%s' % repr(self.opseek)]
        if self.opwrite_same is not None:
            out += ['opwrite_same=%s' % repr(self.opwrite_same)]
        if self.opclone is not None:
            out += ['opclone=%s' % repr(self.opclone)]
        if self.opgetxattr is not None:
            out += ['opgetxattr=%s' % repr(self.opgetxattr)]
        if self.opsetxattr is not None:
            out += ['opsetxattr=%s' % repr(self.opsetxattr)]
        if self.oplistxattrs is not None:
            out += ['oplistxattrs=%s' % repr(self.oplistxattrs)]
        if self.opremovexattr is not None:
            out += ['opremovexattr=%s' % repr(self.opremovexattr)]
...
class nfs_resop4:
    ...
    def __init__(self, resop=None, opaccess=None, opclose=None, opcommit=None, opcreate=None, opdelegpurge=None, opdelegreturn=None, opgetattr=None, opgetfh=None, oplink=None, oplock=None, oplockt=None, oplocku=None, oplookup=None, oplookupp=None, opnverify=None, opopen=None, opopenattr=None, opopen_confirm=None, opopen_downgrade=None, opputfh=None, opputpubfh=None, opputrootfh=None, opread=None, opreaddir=None, opreadlink=None, opremove=None, oprename=None, oprenew=None, oprestorefh=None, opsavefh=None, opsecinfo=None, opsetattr=None, opsetclientid=None, opsetclientid_confirm=None, opverify=None, opwrite=None, oprelease_lockowner=None, opbackchannel_ctl=None, opbind_conn_to_session=None, opexchange_id=None, opcreate_session=None, opdestroy_session=None, opfree_stateid=None, opget_dir_delegation=None, opgetdeviceinfo=None, opgetdevicelist=None, oplayoutcommit=None, oplayoutget=None, oplayoutreturn=None, opsecinfo_no_name=None, opsequence=None, opset_ssv=None, optest_stateid=None, opwant_delegation=None, opdestroy_clientid=None, opreclaim_complete=None, opallocate=None, opcopy=None, opcopy_notify=None, opdeallocate=None, opio_advise=None, oplayouterror=None, oplayputstats=None, opoffload_cancel=None, opoffload_status=None, opread_plus=None, opseek=None, opwrite_same=None, opclone=None, opgetxattr=None, opsetxattr=None, oplistxattrs=None, opremovexattr=None, opillegal=None):
        ...
        self.opallocate = opallocate
        self.opcopy = opcopy
        self.opcopy_notify = opcopy_notify
        self.opdeallocate = opdeallocate
        self.opio_advise = opio_advise
        self.oplayouterror = oplayouterror
        self.oplayputstats = oplayputstats
        self.opoffload_cancel = opoffload_cancel
        self.opoffload_status = opoffload_status
        self.opread_plus = opread_plus
        self.opseek = opseek
    ...
    def __repr__(self):
        ...
        if self.opcopy is not None:
            out += ['opcopy=%s' % repr(self.opcopy)]
        if self.opcopy_notify is not None:
            out += ['opcopy_notify=%s' % repr(self.opcopy_notify)]
        if self.opdeallocate is not None:
            out += ['opdeallocate=%s' % repr(self.opdeallocate)]
        if self.opio_advise is not None:
            out += ['opio_advise=%s' % repr(self.opio_advise)]
        if self.oplayouterror is not None:
            out += ['oplayouterror=%s' % repr(self.oplayouterror)]
        if self.oplayputstats is not None:
            out += ['oplayputstats=%s' % repr(self.oplayputstats)]
        if self.opoffload_cancel is not None:
            out += ['opoffload_cancel=%s' % repr(self.opoffload_cancel)]
        if self.opoffload_status is not None:
            out += ['opoffload_status=%s' % repr(self.opoffload_status)]
...
class offload_info4:
    ...
    # };
    def __init__(self, coa_status=None, coa_resok4=None, coa_bytes_copied=None):
        self.coa_status = coa_status
        self.coa_resok4 = coa_resok4
        self.coa_bytes_copied = coa_bytes_copied

    switch = property(lambda s: {const.NFS4_OK:s.coa_resok4,}.get(s.coa_status, s.coa_bytes_copied))
    ...
    def __repr__(self):
        out = []
        if self.coa_status is not None:
            out += ['coa_status=%s' % const.nfsstat4.get(self.coa_status, self.coa_status)]
        if self.coa_resok4 is not None:
            out += ['coa_resok4=%s' % repr(self.coa_resok4)]
        if self.coa_bytes_copied is not None:
            out += ['coa_bytes_copied=%s' % repr(self.coa_bytes_copied)]
        return 'offload_info4(%s)' % ', '.join(out)
    __str__ = __repr__

class CB_OFFLOAD4args:
    # XDR definition:
    ...
    # };
    def __init__(self, coa_fh=None, coa_stateid=None, coa_offload_info=None):
        self.coa_fh = coa_fh
        self.coa_stateid = coa_stateid
        self.coa_offload_info = coa_offload_info

    def __repr__(self):
        out = []
        if self.coa_fh is not None:
            out += ['coa_fh=%s' % repr(self.coa_fh)]
        if self.coa_stateid is not None:
            out += ['coa_stateid=%s' % repr(self.coa_stateid)]
        if self.coa_offload_info is not None:
            out += ['coa_offload_info=%s' % repr(self.coa_offload_info)]
        return 'CB_OFFLOAD4args(%s)' % ', '.join(out)
    __str__ = __repr__

class CB_OFFLOAD4res:
    # XDR definition:
    # struct CB_OFFLOAD4res {
    #     nfsstat4 cor_status;
    # };
    def __init__(self, cor_status=None):
        self.cor_status = cor_status

    def __repr__(self):
        out = []
        if self.cor_status is not None:
            out += ['cor_status=%s' % const.nfsstat4.get(self.cor_status, self.cor_status)]
        return 'CB_OFFLOAD4res(%s)' % ', '.join(out)
    __str__ = __repr__
...
class nfs_cb_argop4:
    ...
    #         void;
    # };
    def __init__(self, argop=None, opcbgetattr=None, opcbrecall=None, opcblayoutrecall=None, opcbnotify=None, opcbpush_deleg=None, opcbrecall_any=None, opcbrecallable_obj_avail=None, opcbrecall_slot=None, opcbsequence=None, opcbwants_cancelled=None, opcbnotify_lock=None, opcbnotify_deviceid=None, opcboffload=None):
        self.argop = argop
    ...
    def __repr__(self):
        ...
        if self.opcboffload is not None:
            out += ['opcboffload=%s' % repr(self.opcboffload)]
...
class nfs_cb_resop4:
    ...
    switch = property(lambda s: {const.OP_CB_GETATTR:s.opcbgetattr,const.OP_CB_RECALL:s.opcbrecall,const.OP_CB_LAYOUTRECALL:s.opcblayoutrecall,const.OP_CB_NOTIFY:s.opcbnotify,const.OP_CB_PUSH_DELEG:s.opcbpush_deleg,const.OP_CB_RECALL_ANY:s.opcbrecall_any,const.OP_CB_RECALLABLE_OBJ_AVAIL:s.opcbrecallable_obj_avail,const.OP_CB_RECALL_SLOT:s.opcbrecall_slot,const.OP_CB_SEQUENCE:s.opcbsequence,const.OP_CB_WANTS_CANCELLED:s.opcbwants_cancelled,const.OP_CB_NOTIFY_LOCK:s.opcbnotify_lock,const.OP_CB_NOTIFY_DEVICEID:s.opcbnotify_deviceid,const.OP_CB_OFFLOAD:s.opcboffload,const.OP_CB_ILLEGAL:s.opcbillegal,}[s.resop])
...
class open_arguments4:
    ...
    # };
    def __init__(self, oa_share_access=None, oa_share_deny=None, oa_share_access_want=None, oa_open_claim=None, oa_create_mode=None):
        self.oa_share_access = oa_share_access
        self.oa_share_deny = oa_share_deny
        self.oa_share_access_want = oa_share_access_want
        self.oa_open_claim = oa_open_claim
        self.oa_create_mode = oa_create_mode

    def __repr__(self):
        out = []
        if self.oa_share_access is not None:
            out += ['oa_share_access=%s' % repr(self.oa_share_access)]
        if self.oa_share_deny is not None:
            out += ['oa_share_deny=%s' % repr(self.oa_share_deny)]
        if self.oa_share_access_want is not None:
            out += ['oa_share_access_want=%s' % repr(self.oa_share_access_want)]
        if self.oa_open_claim is not None:
            out += ['oa_open_claim=%s' % repr(self.oa_open_claim)]
        if self.oa_create_mode is not None:
            out += ['oa_create_mode=%s' % repr(self.oa_create_mode)]
        return 'open_arguments4(%s)' % ', '.join(out)
    __str__ = __repr__

fattr4_open_arguments = open_arguments4
fattr4_time_deleg_access = nfstime4
fattr4_time_deleg_modify = nfstime4
