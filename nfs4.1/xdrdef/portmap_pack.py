...
import sys,os
from . import portmap_const as const
from . import portmap_type as types
...
class PORTMAPPacker(xdrlib.Packer):
    ...
    def pack_pmaplist(self, data):
        ...
        self.pack_pmaplistptr(data.next)

    def pack_pmaplistptr(self, data):
        if hasattr(self, 'filter_pmaplistptr'):
            data = getattr(self, 'filter_pmaplistptr')(data)
        if len(data) > 1 and self.check_array:
            raise XDRError('array length too long for data')
        self.pack_array(data, self.pack_pmaplist)
...
class PORTMAPUnpacker(xdrlib.Unpacker):
    ...
    def unpack_pmaplist(self):
        ...
        data.next = self.unpack_pmaplistptr()
        if hasattr(self, 'filter_pmaplist'):
            data = getattr(self, 'filter_pmaplist')(data)
        return data

    def unpack_pmaplistptr(self):
        data = self.unpack_array(self.unpack_pmaplist)
        if len(data) > 1 and self.check_array:
            raise XDRError('array length too long for data')
        if hasattr(self, 'filter_pmaplistptr'):
            data = getattr(self, 'filter_pmaplistptr')(data)
